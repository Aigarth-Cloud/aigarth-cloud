...
const MIGRATIONS_TABLE = "work_migrations";
...
console.log(`[work] applying migrations from ${migrationsFolder} (table: ${MIGRATIONS_SCHEMA}.${MIGRATIONS_TABLE})`);
...
try {
  ...
  // eslint-disable-next-line no-console
  console.error(`[work] migration FAILED:`, err);
  await closeDb();
  process.exit(1);
}
await closeDb();
// eslint-disable-next-line no-console
console.log("[work] migrations applied");
